...
describe("InvoiceFactory", () => {
  ...
  describe("Create invoice", () => {
    it("Should mint invoice erc1155", async () => {
      ...
      await expect(
        invoiceFactory.createInvoice(args.client, args.fractions, args.fractionalPrice, args.invoiceURI, args.token),
      )
    ...
    it("Should set invoice on factoring", async () => {
      ...
      expect(invoiceData.repaymentAmount).to.be.equal(0);
